...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True, multi_grid=False,
                 deep_base=False, norm_layer=nn.BatchNorm2d, in_channels=3, output_stride=8):
...
def resnet50(pretrained=False, root='./pretrained', **kwargs):
    ...
    if pretrained:
        ...
        # resnet50_setting = pretrained_settings["resnet50"]
        model.load_state_dict(model_zoo.load_url(pretrained_settings["resnet50"]["imagenet"]["url"]))
    return model
...
def resnet101(pretrained=False, root='./pretrained', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(pretrained_settings["resnet101"]["imagenet"]["url"]))
...
def resnet152(pretrained=False, root='./pretrained', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(pretrained_settings["resnet152"]["imagenet"]["url"]))
